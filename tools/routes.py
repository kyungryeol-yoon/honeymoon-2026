...
ROUTES = {
 ...
 # ── 루가노 ───────────────────────────────────────────────────────────
 # 캠핑장이 호숫가(Agno 만)라 수영·산책은 걸어나가면 됩니다.
 # 시내(Lido · Parco Ciani)는 차로 15분인데, 저녁에 메를롯을 마실 거면
 # Ticino Ticket 으로 FLP 를 타는 편이 낫습니다 — 숙박객은 공짜입니다.
 ("2026-09-21", "18:00"): ("루가노 호수", "캠핑장 앞이 호숫가 · 시내는 렌터카 15분", [
    W(2, "Premium lake pitch 바로 앞 = Agno 만 호숫가"),
 ], [
    "시내로 나가면 Lido di Lugano · Parco Ciani — 렌터카 15분",
    "Ticino Ticket (체크인 때 받음) 이면 Agno역 → FLP 종점 Lugano 까지 무료 · "
    "캠핑장에서 역까지 도보 15분 + 기차 약 18분",
 ]),
